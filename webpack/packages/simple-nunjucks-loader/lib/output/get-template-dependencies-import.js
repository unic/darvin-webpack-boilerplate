...
function foldDependenciesToImports(loaderContext, [imports, assignment], [, fullPath], i) {
  const path = (0, _loaderUtils.stringifyRequest)(loaderContext, fullPath);
  const importVar = `templateDependencies${i}`;
  return [`${imports}var ${importVar} = require(${path}).__nunjucks_module_dependencies__;`, {
...
function getTemplateDependenciesImport(loaderContext, dependencies) {
  return getImports(...dependencies.reduce(foldDependenciesToImports.bind(null, loaderContext), ['', {
    templates: '{}',
    globals: '{}',
    extensions: '{}',
    filters: '{}'
